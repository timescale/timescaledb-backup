...
func preRestoreTimescale(dbURI string, tsInfo util.TsInfo) {
	...
	_, err := conn.Exec(context.Background(), fmt.Sprintf("CREATE EXTENSION IF NOT EXISTS timescaledb WITH SCHEMA %s VERSION '%s'", tsInfo.TsSchema, tsInfo.TsVersion))
	if err != nil {
		log.Fatalf("Error during TimescaleDB extension creation %s\n", err)
	}
